    ...
    <div class="container">
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="list-group">
                            ...
                            <a href="#" class="list-group-item list-group-item-action border-0 rounded active">{{ __('home.users') }}</a>
                            @can('categories')
            ...
            <div class="col-md-8 order-md-1">
                <div class="card mb-4">
                    <h5 class="card-header">{{ __('home.users') }}</h5>
                                    ...
                                    <tbody>
                                        @foreach ($users as $user)
                                            <tr>
                                                @can('update-users')
                                                    <th class="text-nowrap">
                                                        ...
                                                        @foreach ($user->roles as $role)
                                                            <span class="@if ($role->name == 'admin') text-danger @else text-muted @endif" style="font-weight: normal !important">{{ Str::ucfirst($role->name) }}</span>
                                                        @endforeach
                                                    </th>
                                                @else
                                                ...
                                                @endcan
                                                <td>{{ $user->email }}</td>
                                                <td class="text-nowrap text-center">{{ $user->created_at->format('H:i - d M Y') }}</td>
                                                @canany(['update-users', 'delete-users'])
                                                    <td>
